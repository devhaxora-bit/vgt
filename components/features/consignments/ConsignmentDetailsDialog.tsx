...
.hdr { border-bottom: 2px solid #1d2f7a; padding: 8px 10px 28px; background: ${config.paperTint}; }
...
.top-grid { display: grid; grid-template-columns: 1.22fr 1.1fr 1.02fr 0.72fr; gap: 6px; padding: 6px; border-bottom: 1px solid #1d2f7a; align-items: stretch; }
.top-grid > div { display: flex; flex-direction: column; gap: 6px; min-height: 100%; }
.top-grid > div > .box { flex: 1 1 0; min-height: 0; }
.top-grid > .box { height: 100%; }
.top-grid > div:first-child > .box {
    display: flex;
...
.top-grid > div:first-child > .box .lbl {
    font-size: 15px;
    margin-bottom: 2px;
}
...
/* ARC-style faint name watermark — even overlay on consignee (white) copy only */
.name-wm {
    position: absolute;
    inset: 0;
    z-index: 20;
...
.name-wm-line { display: block; width: 200%; margin: 0; padding: 0; }
</style>
</head>
<body>
<div class="page">
    ${type === 'consignee' ? (() => {
        ...
        const line = phrase.repeat(28);
        const lines = Array.from({ length: 100 }, () => `<div class="name-wm-line">${line}</div>`).join('');
