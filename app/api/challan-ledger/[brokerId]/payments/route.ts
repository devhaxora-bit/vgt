...
import { buildSettledChallanBillAmountMap } from '@/lib/server/challanBillingSnapshot';
...
export async function POST(
    ...
    if (normalizedBillingRecordIds.length > 0) {
        ...
        if (!billingRecords || billingRecords.length !== normalizedBillingRecordIds.length) {
            return NextResponse.json({ error: 'One or more selected bills are invalid for this broker' }, { status: 400 });
        }
        ...
        if (normalizedBillAllocations.length > 0) {
            ...
            const alreadySettledMap = buildSettledChallanBillAmountMap(existingReceipts || []);
    ...
    const { data, error } = await supabase
        ...
        .insert({
            ...
            related_billing_record_ids: normalizedBillingRecordIds.length > 0 ? normalizedBillingRecordIds : [],
