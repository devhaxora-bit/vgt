...
BEGIN
  ...
  SELECT bcr.*
  INTO v_target
  FROM public.branch_cn_ranges AS bcr
  WHERE bcr.branch_id = v_branch.id
    AND bcr.status IN ('active', 'pending', 'exhausted')
    AND p_cn_no BETWEEN bcr.range_start AND bcr.range_end
  ORDER BY
    CASE bcr.status WHEN 'active' THEN 0 WHEN 'pending' THEN 1 ELSE 2 END,
    bcr.created_at DESC
  FOR UPDATE
  LIMIT 1;
