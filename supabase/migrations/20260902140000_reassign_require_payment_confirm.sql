-- Do not move payments with a bill unless the caller explicitly confirms.
-- Consignments are never moved.

DROP FUNCTION IF EXISTS public.fn_reassign_billing_record_party(uuid, uuid, uuid);
...
BEGIN
  ...
  IF v_exclusive_count > 0 AND NOT COALESCE(p_confirm_move_payments, false) THEN
    RAISE EXCEPTION 'This bill has % linked payment(s). Confirm moving both the bill and those payments. Consignments will not be moved.',
      v_exclusive_count
      USING ERRCODE = 'P0409';
  END IF;
